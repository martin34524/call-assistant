defmodule CallAssistantWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, CallAssistantWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "sends a fresh admin login straight to /admin, not conn.assigns.current_scope (which is still stale/anonymous at this point in the real controller flow)",
         %{conn: conn} do
    ...
    test "redirects to the member dashboard when user is already logged in", %{
      conn: conn,
      user: user
    } do
      conn =
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      CallAssistantWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: CallAssistantWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        CallAssistantWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
